...
function Cart() {
  ...
  const removeProductFromCart = (productId) => {
    ...
    axios
      .get(BASE_URL + "/ecomm/api/v1/carts/" + cartId, {
        headers: {
      ...
      .then((response) => {
        response.data.productsSelected.forEach((element) => {
          if (productId !== element.id) data.productIds.push(element.id);
        });
        axios
          .put(BASE_URL + "/ecomm/api/v1/carts/" + cartId, data, {
            headers: {
              "x-access-token": localStorage.getItem("token"),
            },
          })
  ...
  const fetchOrderDetails = () => {
    ...
    axios
      .get(BASE_URL + "/ecomm/api/v1/carts/" + localStorage.getItem("cartId"), {
        params: data,
        headers: {
          "x-access-token": localStorage.getItem("token"),
        },
        ...
        <div className="row">
          ...
          <div className="cart-wrapper d-flex flex-row">
            <div className="order-details d-flex flex-column">
              ...
              {orderDetails.productsSelected &&
              orderDetails.productsSelected.length > 0 ? (
                orderDetails.productsSelected.map((product) => (
                  <div
                    className="order-details-product d-flex flex-row"
                    key={product.id}
                  >
                    <div className="order-details-product-img d-flex">
                      <img
                        alt="alt"
                        src="https://img.favpng.com/8/17/0/product-design-clip-art-logo-food-png-favpng-TsCQEsJH2LUYN3d5Q6RzrTsqL.jpg"
                      />
                    </div>
              ...
              orderDetails.productsSelected.length > 0 && (
                <div className="price-details d-flex flex-column">
                  <div className="price-details-box">
                    <div className="price-details-title fw-bold">
                      Price Details
                    </div>
